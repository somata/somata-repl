...
(function() {
  ...
  pipe = new SomataPipeline({
    client: client
  }).use('http').use('exec').use('encodings').use(require('hashpipe/lib/modules/redis').connect()).use({
  ...
  }).set('vars', 'consul_base', client.consul_agent.base_url).alias('deregister', 'val | put $consul_base/catalog/deregister').alias('deregister-all', 'service-nodes || deregister $!').alias('deregister-first', 'service-nodes | head $(length | - 1) || deregister $!');
